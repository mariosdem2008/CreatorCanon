 ...
 *
 * All external dependencies (R2, DB) are stubbed via lightweight in-memory
 * fakes. No network calls, no Trigger.dev runtime. We exercise the core
 * reconciliation rules directly.
 *
 * Rules under test:
 *  1. Rows older than 1h with uploadStatus='uploading' are candidates.
 *  2. Candidate with no localR2Key → mark 'failed'.
 *  3. Candidate whose R2 object EXISTS → mark 'uploaded' (recover).
...
interface SweepDeps {
  ...
  markStatus: (id: string, status: UploadStatus) => Promise<void>;
...
async function runSweepLogic(deps: SweepDeps): Promise<SweepResult> {
  ...
  for (const v of candidates) {
    ...
    try {
      ...
      await deps.markStatus(v.id, 'uploaded');
      recovered++;
    } catch {
      try {
...
function buildDeps(
  ...
  return { deps, statusUpdates, deletedKeys };
}
...
describe('orphaned-uploads-sweep — cleanup path (R2 object missing)', () => {
  ...
  it('still marks failed even when r2Delete throws (best-effort delete)', async () => {
    ...
    const deps: SweepDeps = {
      ...
      markStatus: async (id, status) => { statusUpdates[id] = status; },
