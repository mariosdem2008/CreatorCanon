...
const formatDate = (value: string | null) => {
  ...
  return Number.isNaN(date.getTime()) ? 'Undated' : date.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
};

export function CreatorManualSources({ manifest }: { manifest: CreatorManualManifest; index?: CreatorManualIndex }) {
  return (
    <>
      <section>
        <div className={styles.eyebrow}>Evidence inventory</div>
        <h1 className={styles.pageTitle}>Sources</h1>
        <p className={styles.subtitle}>Original videos and source assets used to ground the manual.</p>
      </section>
      <section className={styles.section}>
        {manifest.sources.length > 0 ? (
          <div className={styles.grid}>
            {manifest.sources.map((source) => (
              <Link key={source.id} href={getCreatorManualSourceRoute(manifest.hubSlug, source.id)} className={styles.recordCard}>
                ...
                <div className={styles.sourceMeta}>
                  ...
                  <span className={styles.sourceMetaItem}>{source.durationSec ? formatCreatorManualTimestamp(source.durationSec) : 'Duration unknown'}</span>
                </div>
              </Link>
            ))}
          </div>
