...
import type { CreatorManualManifest } from './manifest-types';
...
const creatorManualEvidenceRefSchema = z.object({
  sourceId: idSchema,
...
const creatorManualNavItemSchema = z.object({
...
const creatorManualNodeSchema = z.object({
  id: idSchema,
  slug: slugSchema,
...
const creatorManualPillarSchema = z.object({
  id: idSchema,
  slug: slugSchema,
  title: z.string().min(1),
  summary: z.string().min(1),
...
const creatorManualSourceSchema = z.object({
  id: idSchema,
  title: z.string().min(1),
...
const creatorManualSegmentSchema = z.object({
  id: idSchema,
  sourceId: idSchema,
  slug: slugSchema,
  title: z.string().min(1),
...
const creatorManualClaimSchema = z.object({
  id: idSchema,
  title: z.string().min(1),
...
const creatorManualGlossaryEntrySchema = z.object({
  id: idSchema,
  term: z.string().min(1),
  slug: slugSchema,
...
const creatorManualThemeSchema = z.object({
  id: idSchema,
  slug: slugSchema,
  title: z.string().min(1),
  summary: z.string().min(1),
...
const creatorManualWorkshopStageSchema = z.object({
  id: idSchema,
  slug: slugSchema,
  title: z.string().min(1),
  summary: z.string().min(1),
...
const creatorManualSearchDocSchema = z.object({
  id: idSchema,
...
export function parseCreatorManualManifest(
  value: unknown,
  context = 'Creator Manual manifest',
): CreatorManualManifest {
  const parsed = creatorManualManifestSchema.safeParse(value);
  if (!parsed.success) {
    const details = parsed.error.issues
      .slice(0, 6)
      .map((issue) => `${issue.path.join('.') || '<root>'}: ${issue.message}`)
      .join('; ');
    throw new Error(`${context} failed Creator Manual schema validation: ${details}`);
  }

  return parsed.data as CreatorManualManifest;
}
