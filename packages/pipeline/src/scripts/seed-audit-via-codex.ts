...
function buildBriefPrompt(
  ...
  return [
    ...
    '',
    '# VOICE FIDELITY — REQUIRED',
    'Every brief\'s voiceFingerprint.preserveTerms[] declares the creator\'s verbatim phrases that MUST appear in this brief\'s prose. The actual openingHook + audienceQuestion + each outline section\'s intent string must collectively use AT LEAST 60% of the declared preserveTerms verbatim. If you can\'t naturally use 60% of the declared terms, REVISE the preserveTerms list down to terms that genuinely fit — do NOT pad it with terms you didn\'t actually use. Voice fidelity is measured downstream by check-voice-fingerprint.ts; briefs scoring <60% preserve-term hit rate are flagged for editor rework.',
    '',
    '# OUTPUT FORMAT — CRITICAL',
