...
function createEnvModelClient(): CreatorManualDesignSpecModelClient | null {
  if (shouldUseCodexCliForDesignSpec(process.env)) {
...
function shouldUseCodexCliForDesignSpec(env: Record<string, string | undefined>): boolean {
  return env.PIPELINE_QUALITY_MODE === 'codex_dev' || env.PIPELINE_OPENAI_PROVIDER === 'codex_cli';
}

function createCodexCliDesignSpecModelClient(): CreatorManualDesignSpecModelClient {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('Codex CLI design-spec generation is disabled in production.');
  }
  ...
  const argsTemplate = [
    ...
    '--sandbox',
    'read-only',
    ...(process.env.CODEX_CLI_MODEL ? ['--model', process.env.CODEX_CLI_MODEL] : []),
    '-o',
    '<tmpfile>',
  ];

  const provider = createCodexCliProvider({
    binary: process.env.CODEX_CLI_BIN,
    argsTemplate,
